# API Discovery Error Model

**File:** `skills/discovery/api-discovery/error-model.md`
...
This document defines the error model for the API Discovery Skill.
...
- Missing base-path candidate list
- Non-positive bounds
...
SHALL NOT abort other requests.

---

# Parsing Errors

Raised when a located specification cannot be parsed.

Parsing errors SHALL NOT fabricate operations; the specification SHALL be recorded
as located but unparsed, with the error surfaced for diagnosis.

---

# Analysis Errors

Raised when API-exposure analysis cannot complete.
...
A failure affecting one request or specification SHALL NOT abort the discovery.
The overall outcome SHALL be `partial` when some requests did not complete, and
