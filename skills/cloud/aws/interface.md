# AWS Cloud Security Skill Interface

**File:** `skills/cloud/aws/interface.md`
...
This document defines the stable, implementation-independent interface of the AWS Cloud
...
The skill exposes a single logical operation, `assess`, that evaluates AWS security
...
[Assets](../../../schemas/asset.md). `account_scope_ref` references authorized accounts,
regions, and services. `credential_ref` references authorized read credentials by
reference only. `options` toggle individual analyses and default to configuration
defaults.
...
- No out-of-scope account SHALL have been assessed.
...
interface SHALL surface deterministic outcomes and SHALL NOT leak credentials or
provider internals.
