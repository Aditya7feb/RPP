# SAML Authentication Execution Model

**File:** `skills/authentication/saml/execution.md`
...
This document defines the deterministic execution model of the SAML Authentication
Skill, stage by stage. Given the same inputs, configuration, and service-provider
...
before every target-facing action. Assertion testing is an `active` action. Only an
...
# Stage 3 — Observe Assertion Consumption

The skill SHALL observe how the service provider consumes managed test assertion
variants through the [HTTP Client](../../shared/http-client/README.md). Assertion
variants SHALL be derived from managed material, never persisted, and the skill
SHALL NOT perform requests directly.

---

# Stage 4 — Analyze Signature, Restrictions, And Replay

The skill SHALL analyze signature validation, signature wrapping and stripping,
audience restriction, recipient and destination validation, replay protection, and
transport security using deterministic criteria.
...
to [Evidence](../../../schemas/evidence.md). Assertions and signing material SHALL
be redacted.
...
The skill SHALL analyze the observations for SAML weaknesses using deterministic
