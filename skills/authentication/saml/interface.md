...
**File:** `skills/authentication/saml/interface.md`
...
This document defines the stable, implementation-independent interface of the SAML
Authentication Skill. Consumers depend only on this contract.
...
- `target` (required) — an in-scope SAML service provider assertion-consumer
  endpoint.
- `assets` (optional) — the `web-application`, `endpoint`, and `identity`
  [Assets](../../../schemas/asset.md) under test.
- `assertion_ref` (optional) — a reference to a managed test assertion and signing
  material. It SHALL be a reference, never inline material.
...
- `evidence` — [Evidence](../../../schemas/evidence.md) references, assertions and
  signing material redacted.
...
- Observe SAML assertion consumption through the shared HTTP Client
- Record Observations and promote Evidence with assertions and signing material
  redacted
