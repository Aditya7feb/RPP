# CSRF Protection Error Model

**File:** `skills/authentication/csrf/error-model.md`
...
This document defines the canonical error categories of the CSRF Protection Skill
...
Raised when managed credentials fail to reach an authenticated state-changing
endpoint.

- Outcome — `partial`
- Action — evaluate unauthenticated checks; record the failure
...
| Authentication Error | partial | No | Observation ref |
...
- Never execute harmful state changes in response to an error
- Never weaken policy constraints in response to an error
- Redact anti-CSRF tokens in all error evidence
