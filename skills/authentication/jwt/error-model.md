# JWT Authentication Error Model

**File:** `skills/authentication/jwt/error-model.md`
...
This document defines the canonical error categories of the JWT Authentication
...
Raised when a provided test token is malformed or cannot be parsed.

- Outcome — `partial`
- Action — evaluate structure-independent checks; record the failure
- Evidence — the failed observation reference, secrets redacted
...
| Token Error | partial | No | Observation ref |
...
- Never exceed bounded secret-analysis limits in response to an error
- Never weaken policy constraints in response to an error
- Redact token secrets and full tokens in all error evidence
