...
**File:** `skills/authentication/jwt/interface.md`
...
This document defines the stable, implementation-independent interface of the JWT
...
- `token_ref` (optional) — a reference to a managed test token. It SHALL be a
  reference, never an inline token.
...
- `evidence` — [Evidence](../../../schemas/evidence.md) references, token secrets
  redacted.
...
- Observe token handling through the shared HTTP Client
- Bound any secret-recovery check by configuration and Rules of Engagement
- Record Observations and promote Evidence with token secrets redacted
