# Mutual TLS Authentication Error Model

**File:** `skills/authentication/mtls/error-model.md`
...
This document defines the canonical error categories of the Mutual TLS
Authentication Skill and their mapping to outcomes. Errors are classified
deterministically so consumers can respond consistently.
...
Raised when a TLS handshake fails in a way that prevents a specific check.

- Outcome — `partial`
- Action — continue with remaining checks; record the failure
- Evidence — the failed observation reference, key material redacted
...
Raised when a handshake or interaction exceeds the configured timeout.
...
| Handshake Error | partial | No | Observation ref |
...
- Redact private key material in all error evidence
