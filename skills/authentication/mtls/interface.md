# Mutual TLS Authentication Interface

**File:** `skills/authentication/mtls/interface.md`
...
Mutual TLS Authentication Skill. Consumers depend only on this contract.
...
- `target` (required) — an in-scope service endpoint expected to enforce mutual
  TLS.
- `assets` (optional) — the `service`, `endpoint`, and `certificate`
  [Assets](../../../schemas/asset.md) under test.
- `client_certificate_ref` (optional) — a reference to a managed test client
  certificate and key. It SHALL be a reference, never inline key material.
...
- `evidence` — [Evidence](../../../schemas/evidence.md) references, private key
  material redacted.
...
- Observe mutual TLS behavior through the shared TLS and HTTP Clients
- Record Observations and promote Evidence with private key material redacted
