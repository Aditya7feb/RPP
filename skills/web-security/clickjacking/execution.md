# Clickjacking Execution Model

**File:** `skills/web-security/clickjacking/execution.md`
...
This document defines the deterministic execution model of the Clickjacking Skill,
stage by stage. Given the same inputs, configuration, and response behavior,
...
# Stage 3 — Observe Response Framing Controls
...
# Stage 4 — Analyze Framing Protection And Page Sensitivity

The skill SHALL analyze the `X-Frame-Options` header and CSP `frame-ancestors`
directive and assess page sensitivity using deterministic criteria.
...
The skill SHALL analyze the observations for clickjacking weaknesses using
deterministic criteria and classify them using canonical weakness identifiers such
as CWE-1021. Analysis SHALL be separate from observation.
