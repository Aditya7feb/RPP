# Server-Side Request Forgery Error Model

**File:** `skills/web-security/ssrf/error-model.md`
...
This document defines the canonical error categories of the Server-Side Request Forgery
...
Raised when a required action returns `requires_approval`, as SSRF testing commonly
does.
...
Raised when a probe cannot be delivered to a specific request-issuing parameter.

- Outcome — `partial`
- Action — continue with remaining parameters; record the failure
...
- Outcome — `partial`
- Action — record, distinguish from a server-side fetch, and continue
...
| Probe Error | partial | No | Observation ref |
...
- Never reach internal services or sensitive endpoints in response to an error
- Never weaken policy constraints in response to an error
- Redact any incidental sensitive content in all error evidence
