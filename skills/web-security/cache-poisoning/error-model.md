# Web Cache Poisoning Error Model

**File:** `skills/web-security/cache-poisoning/error-model.md`
...
This document defines the canonical error categories of the Web Cache Poisoning Skill
...
Raised when a required action returns `requires_approval`, as cache-poisoning testing
commonly does.
...
Raised when a probe cannot be delivered against the controlled cache key.
...
- Never poison a shared user-facing cache key in response to an error
