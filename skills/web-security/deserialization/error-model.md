# Insecure Deserialization Error Model

**File:** `skills/web-security/deserialization/error-model.md`
...
This document defines the canonical error categories of the Insecure Deserialization
...
Raised when a required action returns `requires_approval`, as deserialization testing
commonly does.
...
Raised when a serialized probe cannot be delivered to a specific endpoint.
...
- Outcome — `partial`
- Action — record, distinguish from processing delay, and continue
...
- Never deliver a functional gadget chain in response to an error
