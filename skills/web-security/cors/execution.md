# CORS Execution Model

**File:** `skills/web-security/cors/execution.md`
...
This document defines the deterministic execution model of the CORS Skill, stage by
stage. Given the same inputs, configuration, and response behavior, execution SHALL
be reproducible.
...
before every target-facing action. Cross-origin probing is an `active` action. Only
...
# Stage 3 — Observe Cross-Origin Responses

The skill SHALL observe cross-origin response headers, probing with the configured
untrusted origins, through the [HTTP Client](../../shared/http-client/README.md),
capturing the canonical [HTTP Header](../../../schemas/http-header.md)
representation. The skill SHALL NOT perform requests directly.

---

# Stage 4 — Analyze Reflection, Credentials, And Permissions

The skill SHALL analyze origin reflection, `null` origin acceptance, credentialed
access, wildcard-with-credentials, and permissive methods and headers using
...
The skill SHALL analyze the observations for CORS weaknesses using deterministic
criteria and classify them using canonical weakness identifiers such as CWE-942.
