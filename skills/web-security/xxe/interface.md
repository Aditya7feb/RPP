# XML External Entity Interface

**File:** `skills/web-security/xxe/interface.md`
...
This document defines the stable, implementation-independent interface of the XML
External Entity Skill. Consumers depend only on this contract.
...
- `target` (required) — an in-scope web application or API base URL that processes
  XML.
- `assets` (optional) — the `web-application`, `endpoint`, and `api`
  [Assets](../../../schemas/asset.md) under test.
- `marker_ref` (optional) — a reference to a non-sensitive resource used to confirm
  resolution.
...
- Submit bounded entity probes and observe resolution through the shared HTTP Client
