# SQL Injection Error Model

**File:** `skills/web-security/sqli/error-model.md`
...
This document defines the canonical error categories of the SQL Injection Skill and
their mapping to outcomes. Errors are classified deterministically so consumers can
...
Raised when a probe cannot be delivered to a specific injection point.
...
Raised when an interaction exceeds the configured timeout, which may confound
time-based signals.

- Outcome — `partial`
- Action — record, distinguish from induced delay, and continue
...
- Never extract or alter data in response to an error
