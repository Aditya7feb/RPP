...
**File:** `skills/web-security/xss/interface.md`
...
Cross-Site Scripting Skill. Consumers depend only on this contract.
...
- `payload_set_ref` (optional) — a reference to a managed set of bounded marker
...
- Inject bounded marker payloads and observe rendering through the shared HTTP
  Client and Browser
- Confirm execution with non-destructive markers only
...
`denied` status; a pending approval yields `awaiting_approval`; transport or
rendering failures yield `partial` or `error` with Evidence where available.
