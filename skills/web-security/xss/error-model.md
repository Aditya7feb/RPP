# Cross-Site Scripting Error Model

**File:** `skills/web-security/xss/error-model.md`
...
This document defines the canonical error categories of the Cross-Site Scripting
...
Raised when a required action returns `requires_approval`, including stored-XSS
testing where the Rules of Engagement require approval.
...
Raised when a marker cannot be delivered to a specific injection point.

- Outcome — `partial`
- Action — continue with remaining injection points; record the failure
...
Raised when the Browser cannot render a response to observe execution.

- Outcome — `partial`
- Action — fall back to reflection analysis; record the failure
- Evidence — the failed observation reference
...
| Injection Error | partial | No | Observation ref |
| Rendering Error | partial | No | Observation ref |
...
- Never deliver weaponized payloads in response to an error
- Never weaken policy constraints in response to an error
- Redact sensitive surrounding content in all error evidence
