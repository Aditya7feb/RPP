# Unrestricted File Upload Interface

**File:** `skills/web-security/file-upload/interface.md`
...
Unrestricted File Upload Skill. Consumers depend only on this contract.
...
```
assess_file_upload(request) → result
```
...
- `target` (required) — an in-scope web application or API base URL with upload
  functionality.
- `assets` (optional) — the `web-application`, `endpoint`, and `api`
  [Assets](../../../schemas/asset.md) under test.
- `marker_set_ref` (optional) — a reference to a managed set of inert marker files. It
...
- `status` — one of `completed`, `partial`, `denied`, `awaiting_approval`,
  `error`.
- `metrics` — counters such as upload endpoints tested and findings emitted.
...
- Submit inert marker files and observe validation through the shared HTTP Client
- Confirm weak validation with inert markers only
