# Unrestricted File Upload Execution Model

**File:** `skills/web-security/file-upload/execution.md`
...
This document defines the deterministic execution model of the Unrestricted File
Upload Skill, stage by stage. Given the same inputs, configuration, and application
...
before every target-facing action. Upload probing is an `active`, higher-impact
...
# Stage 3 — Submit Inert Marker Files

For authorized upload endpoints the skill SHALL submit inert, non-executable marker
files drawn from the managed set through the
[HTTP Client](../../shared/http-client/README.md). The skill SHALL NOT upload
functional malicious payloads and SHALL NOT perform requests directly.

---

# Stage 4 — Analyze Type, Content, And Storage Validation

The skill SHALL analyze whether type validation is adequate, whether content is
validated beyond declared type, and whether uploaded content is stored web-accessibly
or served with an unsafe content type, using deterministic criteria.
...
to [Evidence](../../../schemas/evidence.md). Only the inert marker upload SHALL be
recorded, and sensitive content SHALL be redacted.

---

# Stage 6 — Analyze For File Upload Weaknesses

The skill SHALL analyze the observations for unrestricted file upload using
deterministic criteria and classify them using canonical weakness identifiers such as
CWE-434. Analysis SHALL be separate from observation.
...
- Confirmation uses only inert, non-executable markers.
