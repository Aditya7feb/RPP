...
**File:** `skills/web-security/path-traversal/interface.md`
...
This document defines the stable, implementation-independent interface of the Path
Traversal Skill. Consumers depend only on this contract.
...
```
assess_path_traversal(request) → result
```
...
# Operation: assess_path_traversal
...
- `marker_ref` (optional) — a reference to a non-sensitive marker resource used to
  confirm traversal.
...
- `status` — one of `completed`, `partial`, `denied`, `awaiting_approval`,
  `error`.
- `metrics` — counters such as path parameters tested and findings emitted.
...
- Inject bounded traversal probes and observe responses through the shared HTTP
  Client
