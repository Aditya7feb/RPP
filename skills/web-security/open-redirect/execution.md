# Open Redirect Execution Model

**File:** `skills/web-security/open-redirect/execution.md`
...
This document defines the deterministic execution model of the Open Redirect Skill,
stage by stage. Given the same inputs, configuration, and redirect behavior,
...
before every target-facing action. Redirect probing is an `active` action. Only an
...
# Stage 3 — Observe Redirect Responses

The skill SHALL observe redirect responses, supplying a benign controlled
destination through candidate parameters, through the
[HTTP Client](../../shared/http-client/README.md), capturing the canonical
[HTTP Redirect](../../../schemas/http-redirect.md) representation. The skill SHALL
NOT follow the redirect into a harmful destination and SHALL NOT perform requests
directly.

---

# Stage 4 — Analyze Redirect Parameters And Destination Validation

The skill SHALL analyze which parameters control the redirect target and whether
destinations are validated against trusted origins using deterministic criteria.
...
to [Evidence](../../../schemas/evidence.md). Sensitive tokens in observed targets
SHALL be redacted.
...
The skill SHALL analyze the observations for open-redirect weaknesses using
deterministic criteria and classify them using canonical weakness identifiers such
as CWE-601. Analysis SHALL be separate from observation.
