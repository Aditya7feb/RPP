...
**File:** `skills/shared/azure-client/configuration.md`
...
This document defines the declarative configuration of the Azure Client Shared Skill and
...
azure_client:
  scope:
    ...
    regions:
    services:
    cross_subscription_authorized: false
  ...
  authentication:
    method: service_principal | managed_identity | access_token
    secret_ref:
...
`subscriptions`, `resource_groups`, `regions`, and `services` enumerate authorized scope.
`cross_subscription_authorized` SHALL default to `false`. Operations outside scope SHALL
be rejected.
...
`observe_imds` enables observation of the Azure Instance Metadata Service; observations are
reported as data.
...
- `subscriptions`, `resource_groups`, and `services` SHALL be non-empty for active
  operations.
