# Azure Client Execution

**File:** `skills/shared/azure-client/execution.md`
...
This document defines the deterministic execution model of the Azure Client Shared Skill,
...
The client SHALL validate the operation and confine it to authorized subscriptions,
resource groups, regions, and services. An operation outside scope SHALL be rejected
before any request.
...
[Secrets Client](../secrets-client/README.md). Credentials SHALL NOT be logged or placed
in evidence.
...
mutation. A `requires_approval` decision SHALL defer it; a `deny` decision SHALL suppress
it.
...
The client SHALL observe role, network, configuration, and instance-metadata data as
...
Read operations SHALL NOT alter subscription state. Mutations SHALL occur only when gated
and authorized, and SHALL be recorded.
