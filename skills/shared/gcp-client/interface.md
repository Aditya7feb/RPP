...
**File:** `skills/shared/gcp-client/interface.md`
...
The GCP Client Interface defines the canonical contract through which platform components
interact with GCP service and metadata endpoints.
...
All consumers SHALL perform GCP access exclusively through this interface.
...
HTTP Client (TLS) → GCP Service APIs
```
...
`service` SHALL name the GCP service. Targets SHALL be within authorized scope.
...
```yaml
operation:
  kind: get | list | create | update | delete | set-iam-policy
  resource_type:
  selectors:
  pagination:
    max_items:
...
`get` and `list` are read operations. `create`, `update`, `delete`, and `set-iam-policy`
are intrusive and SHALL be gated through the Policy Engine.
