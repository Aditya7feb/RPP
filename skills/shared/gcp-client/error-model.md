# GCP Client Error Model

**File:** `skills/shared/gcp-client/error-model.md`
...
This document defines the canonical error categories of the GCP Client Shared Skill and
...
|----------|---------|---------|
| out-of-scope | Organization, project, region, or service outside scope | rejected |
...
| throttled | Rate ceiling or service quota reached | retry-or-partial |
...
When a mutation requires approval, the client SHALL defer it to an `awaiting_approval` state
...
When a rate ceiling or service quota is reached, the client MAY retry within limits;
...
When the [HTTP Client](../http-client/README.md) cannot complete a request, the client MAY
