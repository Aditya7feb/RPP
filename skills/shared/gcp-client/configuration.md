...
**File:** `skills/shared/gcp-client/configuration.md`
...
This document defines the declarative configuration of the GCP Client Shared Skill and the
...
gcp_client:
  scope:
    ...
    regions:
    services:
    cross_project_authorized: false
  ...
  metadata:
    observe_metadata_server: true

  authentication:
    method: service_account_key | workload_identity | access_token
    secret_ref:
...
`organizations`, `projects`, `regions`, and `services` enumerate authorized scope.
`cross_project_authorized` SHALL default to `false`. Operations outside scope SHALL be
rejected.
...
`observe_metadata_server` enables observation of the GCP metadata server; observations are
reported as data.
...
- `projects` and `services` SHALL be non-empty for active operations.
