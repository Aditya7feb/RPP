# SOAP API Security Interface

**File:** `skills/api-security/soap/interface.md`
...
This document defines the stable, implementation-independent interface of the SOAP API
Security Skill. Consumers depend only on this contract.
...
```
assess_soap_api(request) → result
```
...
- `target` (required) — an in-scope SOAP service endpoint URL.
...
- `wsdl_ref` (optional) — a reference to a discovered WSDL.
...
- Submit bounded operations using authorized controlled identities through the shared
  HTTP Client
