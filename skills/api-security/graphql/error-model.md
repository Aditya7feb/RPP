# GraphQL API Security Error Model

**File:** `skills/api-security/graphql/error-model.md`
...
This document defines the canonical error categories of the GraphQL API Security Skill
...
Raised when a required action returns `requires_approval`.
...
Raised when a controlled test identity cannot be established for authorization
testing.

- Outcome — `partial`
- Action — evaluate identity-independent checks; record the failure
- Evidence — the failed observation reference, credentials redacted
...
Raised when an interaction exceeds the configured timeout, which may indicate an
unbounded query and SHALL be treated cautiously.

- Outcome — `partial`
- Action — record, avoid escalating query cost, and continue
...
| Identity Error | partial | No | Observation ref |
...
- Never escalate query depth or complexity in response to an error
- Never weaken policy constraints in response to an error
- Redact credentials and sensitive content in all error evidence
