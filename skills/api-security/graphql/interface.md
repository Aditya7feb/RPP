# GraphQL API Security Interface

**File:** `skills/api-security/graphql/interface.md`
...
This document defines the stable, implementation-independent interface of the GraphQL
...
```
assess_graphql_api(request) → result
```
...
- `target` (required) — an in-scope GraphQL API endpoint URL.
...
- `status` — one of `completed`, `partial`, `denied`, `awaiting_approval`,
  `error`.
- `metrics` — counters such as queries tested and findings emitted.
...
- Submit bounded queries using authorized controlled identities through the shared
  HTTP Client
