# REST API Security Interface

**File:** `skills/api-security/rest/interface.md`
...
This document defines the stable, implementation-independent interface of the REST
API Security Skill. Consumers depend only on this contract.
...
```
assess_rest_api(request) → result
```
...
- `target` (required) — an in-scope REST API base URL.
- `assets` (optional) — the `api` and `endpoint`
  [Assets](../../../schemas/asset.md) under test.
- `identities_ref` (optional) — a reference to two authorized controlled identities.
  It SHALL be a reference, never inline credentials.
- `specification_ref` (optional) — a reference to a discovered API specification.
...
- `status` — one of `completed`, `partial`, `denied`, `awaiting_approval`,
  `error`.
- `metrics` — counters such as operations tested and findings emitted.
...
- Exercise operations using authorized controlled identities through the shared HTTP
  Client
