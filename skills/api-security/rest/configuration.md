...
**File:** `skills/api-security/rest/configuration.md`
...
This document defines the declarative configuration of the REST API Security Skill and
...
- `object_level_authorization` — whether object level authorization is checked.
  Default `true`.
- `function_level_authorization` — whether function level authorization is checked.
  Default `true`.
- `mass_assignment` — whether mass assignment is checked. Default `true`.
- `excessive_data_exposure` — whether excessive data exposure is checked. Default
  `true`.
- `resource_consumption` — whether resource-consumption controls are checked. Default
  `true`.
...
- `specification_ref` — a reference to a discovered API specification that enumerates
